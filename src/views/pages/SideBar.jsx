...
const SideBar = () => {
  ...
  useEffect(() => {
    ...
    isAuthenticated().then(val => {
      if (val) dispatch(update(val));
      dispatch(updateLoader(false));
    });
  }, []);

  return (
    <div ref={ref} onClick={(e) => e.stopPropagation()} id='sidebar' className={`sidebar border-gray-200 min-w-[250px] w-[20vw] max-w-[300px] h-screen z-10 bg-[var(--primary-color)] relative ${sidebar.show ? 'sidebar-small-show' : ''}`}>
      <h2 className=' w-full h-[70px] text-4xl font-bold flex items-center justify-between text-white bg-[var(--primary-color)] px-[10px] border-b-1 border-gray-400'>QnA <i className="fa-solid fa-xmark scale-[0.7]"></i></h2>
